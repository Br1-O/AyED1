        ...
        case 'E':
            ...
            //■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■ 
                case 0:
                {
                    ...
                    cout << "Hacer un programa en C++ para un bazar que tiene una promo de descuento para vender al mayor," 
                    ...
                    int quantityProducts, finalPrice;
                    const int PRICE = 80;

                    inputValidation(quantityProducts, "Por favor, ingrese la cantidad de productos que desea comprar: ", 
                    "¡Por favor, ingrese solo valores numericos enteros!");

                    auto applyDiscount = [PRICE](int quantityProducts)
                    {
                        int priceWithDiscount;
                        int fullPrice = quantityProducts * PRICE;

                        if (quantityProducts <= 10)
                        {
                            priceWithDiscount = fullPrice;
                        }
                        else if ( (quantityProducts < 20) && (quantityProducts < 30) )
                        {
                            priceWithDiscount = fullPrice - (fullPrice * 0.25);
                        }
                        else
                        {
                            priceWithDiscount = fullPrice - (fullPrice * 0.4);
                        }

                        return priceWithDiscount;
                    };

                    finalPrice = applyDiscount(quantityProducts);

                    cout << "El total a pagar es de: $ " << finalPrice << endl << endl;
